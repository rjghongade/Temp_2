      ...
      <div className="max-w-7xl mx-auto px-6 text-center">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-8">
          {data.amenities.amenities.map((amenity) => (
            ...
            >
              <div className="w-24 h-24 rounded-full overflow-hidden border-4 border-amber-500 mb-4 shadow-md">
                <img
              ...
              <h3 className="text-xl font-semibold text-amber-400 mb-2">{amenity.amenity_name}</h3>
              <p className="text-green-700 text-sm">Updated: {new Date(amenity.updated_at).toLocaleDateString()}</p>
            </div>
          ))}
        </div>
      </div>
